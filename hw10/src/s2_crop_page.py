...
def parse_args():
    ...
    parser.add_argument(
        "--id", help="To rotated student id", default="111C52032", type=str
    )
...
def getBoundingBox(mask):
    ...
    result = np.array([[w - 1, h - 1], [0, 0]])  # 左上  # 右下
...
def setPointImageFromPath(args) -> str:
    ...
    for contour in contours:
        ...
        if area > min_area and area < max_area:
            ...
            # 過濾掉非正方形以及QRCode的框
            if (
                (ratio := rect_w / rect_h) < 0.9
                or ratio > 1.1
                or rect_y + rect_h > h * 0.95
            ):
                continue

            # 過濾掉框内的雜訊
            dilateMask[
                rect_y + border : rect_y + rect_h - border,
                rect_x + border : rect_x + rect_w - border,
            ] = 0
            globalMask[rect_y : rect_y + rect_h, rect_x : rect_x + rect_w] = dilateMask[
    ...
    if show:
        ...
        cv2.imshow(
            "Global Mask", cv2.resize(mask, (mask.shape[1] // 8, mask.shape[0] // 8))
        )
    ...
    for j in range(10):
        ...
        for k in range(10):
            ...
            if (
                ...
                and twoPointDistance(
                    word_result[0], (word_result[1][0], word_result[0][1])
                )
                - block
                < 10
            ):
                ...
                word_img = word_img[
                    word_result[0][1] + SCALE : word_result[1][1] - SCALE,
                    word_result[0][0] + SCALE : word_result[1][0] - SCALE,
                ]
            else:
                # 採用計算得到的座標(準度較差)
                scale = SCALE + 20
            ...
            if index > 665 and index <= 13725:
                ...
                savePNG(finalWordImg, index, now_page, IM_DIR, unicode)
            else:
                if word_img.shape[0] == 0 or word_img.shape[1] == 0:
                    return f"CropError: {now_page}, code_{str(unicode[index-1])}"
                savePNG(
                    cv2.resize(word_img, (300, 300), interpolation=cv2.INTER_AREA),
                    index,
                    now_page,
                    IM_DIR,
                    unicode,
...
def outputResult(PAGE_START, PAGE_END, results, time):
    ...
    # 輸出執行結果
    print(
        f"The crop of page {PAGE_START}~{PAGE_END} has been completed in {time / 60:.2}mins"
    )
...
def main(args):
    ...
    PAGE_START = input(
        "Please enter the number of pages you want to start processing(default:1): "
    )
    if not PAGE_START.strip():
        PAGE_START = 1
    PAGE_END = input(
        "Please enter the number of pages you want to end processing(default:138): "
    )
    ...
    total_available_code = len(unicode[start_unicode:end_unicode]) - unicode[
        start_unicode:end_unicode
    ].count("123")
